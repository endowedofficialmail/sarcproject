...
import { adminCreateSchoolSchema, getZodErrorMessage } from "@/app/api/_lib/validation";
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = adminCreateSchoolSchema.safeParse({
    ...
    const { name, address, contact_info, email, temporary_password } = parsed.data;
    ...
    if (authCreateError || !authUserResult.user) {
      return NextResponse.redirect(
        new URL(
          `/admin/schools?error=${encodeURIComponent(authCreateError?.message ?? "Failed to create auth user.")}`,
          request.url,
        ),
      );
    }
    ...
    if (schoolError || !school) {
      ...
      return NextResponse.redirect(
        new URL(
          `/admin/schools?error=${encodeURIComponent(schoolError?.message ?? "Failed to create school.")}`,
          request.url,
        ),
      );
    }
    ...
    if (profileError) {
      ...
      await admin.auth.admin.deleteUser(authUserId);
      return NextResponse.redirect(
        new URL(`/admin/schools?error=${encodeURIComponent(profileError.message)}`, request.url),
      );
    }
